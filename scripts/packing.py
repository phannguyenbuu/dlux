...
def write_pack_png(
    ...
    *,
    ...
    effective_bleed = int(round(config.PACK_BLEED * scale))
    if effective_bleed > 0:
        ...
        for zid, (dx, dy) in zone_shift.items():
            ...
            ang = zone_rot.get(zid, 0.0)
            cx, cy = zone_center.get(zid, (0.0, 0.0))
            for pts in zone_exteriors.get(zid, []):
                rpts = _rotate_pts(pts, ang, cx, cy)
                pts_shifted = np.array(
                    [[(p[0] + dx) * hi_scale, (p[1] + dy) * hi_scale] for p in rpts],
                    dtype=np.int32,
            ...
            zone_img = cv2.bitwise_and(img, img, mask=mask)
            cur_mask = mask
            cur_img = zone_img
            for _ in range(effective_bleed):
                next_mask = cv2.dilate(cur_mask, kernel, iterations=1)
                ring = cv2.subtract(next_mask, cur_mask)
                if not np.any(ring):
                    break
                next_img = cv2.dilate(cur_img, kernel, iterations=1)
                img[ring > 0] = next_img[ring > 0]
                alpha[ring > 0] = 255
                cur_mask = next_mask
                cur_img = next_img
